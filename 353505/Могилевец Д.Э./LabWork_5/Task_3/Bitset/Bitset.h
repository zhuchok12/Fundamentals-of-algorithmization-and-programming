...
private:
    ...
    public:
        Bit(uint64_t* block_, size_t pos_) : block(block_), pos(pos_ % 64) {}

        bool test() const { return (*block >> pos) & 1; }
...
public:
    ...
    Bit at(size_t pos) const { return Bit(const_cast<uint64_t*>(&blocks[pos / 64]), pos % 64); }
