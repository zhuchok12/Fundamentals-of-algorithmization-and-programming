    ...
    , ui(new Ui::MainWindow)
{
    ...
    button->setGeometry(300,350,300,300);
    button->setText("Рассчитать");

    informationEdit->setParent(this);
    informationEdit->setGeometry(0, 40, 1500, 30);
    informationEdit->setValidator(val);

    ansEdit->setParent(this);
    ansEdit->setGeometry(0, 100, 2000, 30);
    ansEdit->setEnabled(false);
    ansEdit->setText("Ответ: ");

    QPushButton::connect(button, SIGNAL(pressed()), this, SLOT(on_button_clicked()));

    my_close = new QPushButton("Закрыть",this);
    my_close->setGeometry(650,350,300,300);
    connect(my_close,SIGNAL(clicked(bool)),SLOT(close()));
}
