...
void MainWindow::interpolationSort(QVector<long long>& arr, QString & text) {
    ...
    while (!divideSize.empty()) {
        ...
        printArr(arr, "sort, шаг " + QString::number(step++) + ": ", text);
    }
}
...
void MainWindow::on_pushButton_enter_clicked()
{
    if (ui->Message->text() == "Введите n:"){
        ...
        ui->Message->setText("Введите степень");
        ui->spinBox->setMinimum(1);
    } else if (ui->Message->text() == "Введите mod"){
        ...
        if (bp != -1){
            ui->label_ans->setText("Ответ: " + QString::number(binpow(bp % mod, n % mod, mod)));
        } else {
            ui->label_ans->setText("Ответ:не найден в массиве");
        }
    }
    else{
