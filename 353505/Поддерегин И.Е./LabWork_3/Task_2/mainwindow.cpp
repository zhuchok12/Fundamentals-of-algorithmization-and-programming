...
void MainWindow::calculateAckermann()
{
    ...
    if (m > 3)
    {
        QMessageBox::warning(this, "Ошибка", "Значение m должно быть не больше 3 (функция растёт очень быстро)");
        return;
    }

    if (n > 9)
    {
        QMessageBox::warning(this, "Ошибка", "Значение n должно быть не больше 9(функция растёт очень быстро)");
        return;
    }
