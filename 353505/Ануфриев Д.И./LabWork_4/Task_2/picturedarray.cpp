...
void PicturedArray::interpolationSort(QVector <std::pair<int, QColor>> &sequence, int size) {
    if (size <= 0) {
        return;
    }
    int start = 0;
    ...
    int i = 0;
    int j = 0;
    // Определение переменных min и max
    int min = 0;
    int max = 0;
    std::vector<int> process;
    QVector<QVector <std::pair<int, QColor>>> bucket(size, QVector <std::pair<int, QColor>>());
    // Добавление размера массива
    process.push_back(size);
    while (!process.empty()) {
        // Получение последнего размера
        location = process.back();
        // Удаление последнего элемента из process
        process.pop_back();
        // Получение начального местоположения
        start = end - location;
        // Получение начального элемента
        max = sequence[start].first;
        min = sequence[start].first;
        // Нахождение минимума и максимума
        for (i = start + 1; i < end; i++) {
            if (sequence[i].first > max) {
                max = sequence[i].first;
            } else if (sequence[i].first < min) {
                min = sequence[i].first;
            }
        }
        if (min == max) {
            ...
            for (i = start; i < end; ++i) {
                // Расчет слота
                slot = std::floor(((sequence[i].first - min) / (double)(max - min)) * (location - 1));
                bucket[slot].push_back(sequence[i]);
            }
            for (i = 0; i < location; ++i) {
                if (!bucket[i].empty()) {
                    // Когда слот ведра не пустой
                    // Присваивание элемента ведра в фактический массив
                    for (j = 0; j < bucket[i].size(); j++) {
                        resetColors();
                        sequence[start] = bucket[i][j];
                        sequence[start].second = QColor(255,127,80);;
                    ...
                    // Это полезно для отслеживания элементов сортировки слота
                    process.push_back(bucket[i].size());
                }
            }
            // Удаление существующего элемента ведра
            for (i = 0; i < size; ++i) {
                bucket[i].clear();
            }
        }
    ...
    if(sequence == array) resetColors();
}

void PicturedArray::interpolationSortForTime(QVector<std::pair<int, QColor> > &sequence, int size)
{
    if (size <= 0) {
        return;
    }
...
long long PicturedArray::timeInterpolationSort()
{
    copyOfArray.resize(array.size());
    ...
    interpolationSortForTime(copyOfArray, copyOfArray.size());
