...
Star5::Star5(QVector<QPointF> temp, QGraphicsObject *parent) : Figure(parent)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        points->append(QPointF(center.x() + radius * cos(2 * M_PI * i / 10 - M_PI/2),
                               center.y() + radius * sin(2 * M_PI * i / 10 - M_PI/2)));
    }

    base = abs(points->at(3).x() - points->at(11).x());
    ...
    square = 10 * (sqrt(temp_perim / 2 * (temp_perim / 2 - tri_side) * (temp_perim / 2 - base) * (temp_perim / 2 - side / 2)));
