...
Star8::Star8(QPointF point, QObject *parent) : Figure(point,parent)
{
    Q_UNUSED(point)
}
...
void Star8::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        star << QPointF(getStartPoint().x() + radius * cos(2 * M_PI * i / 16 - M_PI/2),
...
QRectF Star8::boundingRect() const
{
    ...
    for (int i = 0; i < 16; ++i) {
        qreal radius = (i % 2 == 0) ? side : side / 2.0;
        qreal x = getStartPoint().x() + radius * cos(2 * M_PI * i / 16 - M_PI/2);
        qreal y = getStartPoint().y() + radius * sin(2 * M_PI * i / 16 - M_PI/2);
...
qreal Star8::getSFigure() const
{
    return sFigure;
}

qreal Star8::getPFigure() const
{
    return pFigure;
}
