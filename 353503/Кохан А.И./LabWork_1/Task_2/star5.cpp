...
Star5::Star5(QPointF point, QObject *parent) : Figure(point,parent)
{
    Q_UNUSED(point)
}
...
void Star5::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        star << QPointF(getStartPoint().x() + radius * cos(2 * M_PI * i / 10 - M_PI/2),
                        getStartPoint().y() + radius * sin(2 * M_PI * i / 10 - M_PI/2));
    }
...
QRectF Star5::boundingRect() const
{
    ...
    for (int i = 0; i < 10; ++i) {
        qreal radius = (i % 2 == 0) ? side : side / 2.0;
        qreal x = getStartPoint().x() + radius * cos(2 * M_PI * i / 10 - M_PI/2);
        qreal y = getStartPoint().y() + radius * sin(2 * M_PI * i / 10 - M_PI/2);
...
qreal Star5::getSFigure() const
{
    return sFigure;
}

qreal Star5::getPFigure() const
{
    return pFigure;
}
