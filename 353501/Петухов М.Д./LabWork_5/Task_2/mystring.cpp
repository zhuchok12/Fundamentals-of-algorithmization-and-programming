...
void String::reserve(size_t size)
{
    ...
    array = std::move(temp);//array теперь будет владеть памятью, ранее принадлежавшей 
                            //temp. После этого, temp больше не владеет этой памятью, и его деструктор не освободит
                            // эту память.
    capacity_ = size;
}
