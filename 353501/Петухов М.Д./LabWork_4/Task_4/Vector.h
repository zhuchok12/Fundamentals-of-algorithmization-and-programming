...
void Vector<T>::reserve(size_t n){
    ...
    //T* newarr = new T[n];  это плохая реализация reserve, тк мы не хотим чтобы при создании каждого объекта 
    //вызывался конструктор по умолчанию, которого кстати, может и не быть 
    ...
    for(size_t i = 0; i < size; ++i){
        ...
        // вызоы=в конструктора T по адресу newarr + i, даже если там не лежит скноструированного объекта, он все равно сработает 
    }
    for(size_t i = 0; i < size; i++){
        (arr + i)->~T();//явный вызов деструкторов, тк в массиве могут лежать непроинециализированные с помощью конструкторов объекты
    }
    delete[] reinterpret_cast<int8_t*>(arr);//чтобы не вызывать деструкторы, тк это просто числа
    arr = newarr;
    capacity = n;
