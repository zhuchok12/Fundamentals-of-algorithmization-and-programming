...
void RectArray::initializeArray(int n) {
    toSort = generateRandomArray(n);
}

void RectArray::colorizeArray(int n) {
    color.resize(n, 0); // Initialize color vector with all elements as 0 (gray)
}
...
void RectArray::finalizeSortAnimation() {
    for (int i = 0; i < number; i++) {
        color[i] = 2;
        update();
        QCoreApplication::processEvents();
...
void RectArray::merge(QVector<T>& arr, int left, int mid, int right) {
    ...
    while (i < n1 && j < n2) {
        if (leftArr[i] <= rightArr[j]) {
            swap(arr[k], leftArr[i], k, left + i);
            i++;
        } else {
            swap(arr[k], rightArr[j], k, mid + 1 + j);
            j++;
        }
        k++;
    }
