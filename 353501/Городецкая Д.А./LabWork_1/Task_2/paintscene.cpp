...
QString PaintScene::ChangeLabel() {
  ...
  return QString("Center of mass= (%0, %1). Square = %2. Perimeter = %3.")
...
void PaintScene::mousePressEvent(QGraphicsSceneMouseEvent *event) {
  ...
  case otherfigureType: {
    otherfigure *item = new otherfigure(event->scenePos());
    item->setPos(event->pos());
    tempFigure = item;
    break;
  }
  case BrushType: {
    Brush *item = new Brush(event->scenePos());
    item->setPos(event->pos());
    tempFigure = item;
    break;
  }
