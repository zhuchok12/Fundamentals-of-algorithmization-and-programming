...
{
    act=1;
    QPoint delta=MouseMoveNow-MouseMovePress; //Насколько мувнули
    //qDebug()<<"Move to"<<MouseMoveNow;
    //qDebug()<<"Move from"<<MouseMovePress;
    for(auto& a:points)
        a+=delta;
    if(!points.empty())
    center+=delta;
...
void figure::SizeEdit(bool Minus)
{
    ...
    qDebug()<<size;
    resize();
   // findSquare();void on_PlusButton_released();
   // findPerimetr();
}
