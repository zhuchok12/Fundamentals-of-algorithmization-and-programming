    ...
    , ui(new Ui::MainWindow)
{
    ...
    *mainUpd = false;
    ...
    nextpushButton->setText("Сортировать");
    nextpushButton->setGeometry(400, 960, 200, 30);

    spinBox->setParent(this);
    spinBox->setGeometry(0, 960, 200, 30);
    spinBox->setRange(1, 13);

    table->setParent(this);



    QPushButton::connect(nextpushButton, &QAbstractButton::pressed, this, &MainWindow::on_NextpushButton_clicked);
    ...
    *upd = true;
    ...
    connect(tm, &QTimer::timeout, [=](){
        ...
        if (circleFigure[circleFigure.size() - vec[index][0]]->y() <= 400 ){
            if (vec[index][1] < vec[index][2]) speedX1 = 60;
            else speedX1 = -60;
            speedY1 = 0;
        }
        ...
        if (!*upd){
            ...
            for (int i = 0; i < table->rowCount(); ++i){
                if (index == i){
                    ...
                    table->item(i, 1)->setBackground(Qt::black);
                    table->item(i, 2)->setBackground(Qt::black);
                }
            }
